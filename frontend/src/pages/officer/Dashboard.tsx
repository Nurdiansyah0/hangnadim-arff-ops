...
export default function OfficerDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Fetch data individually to prevent one failure (e.g. 404 fitness) from blocking others
      try {
        const fitnessRes = await api.get('/fitness/me/trend');
        setTrends(fitnessRes.data);
      } catch (e) {
        console.log("No fitness data yet");
      }
      ...
      try {
        ...
        console.error("Failed fetching context", e);
      }

      try {
        const tasksRes = await api.get(`/tasks/my-tasks?date=${today}`);
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {!trends?.current ? (
            <div className="bg-slate-900/50 border border-slate-800 rounded-4xl p-12 text-center">
              ...
              <p className="text-slate-500 italic text-sm">Battery Test data not available yet.</p>
            </div>
          ) : (
          ...
          <div className="bg-slate-900/40 border border-slate-800 rounded-4xl overflow-hidden">
            {tasks.length === 0 ? (
              <div className="p-8 text-center">
                <p className="text-slate-500 text-sm italic">No tasks scheduled for today.</p>
              </div>
            ) : (
