...
impl ApprovalService {
    ...
    pub async fn transition_inspection(
        ...
        match (current_status, target_status, role_id) {
            ...
            ("DRAFT", "SUBMITTED", 4) | ("DRAFT", "SUBMITTED", 1) => self
                .repo
                .update_inspection_status(inspection_id, "SUBMITTED", user_id)
                .await
                .map_err(|e| e.to_string()),
            // TL meriview laporan
            ("SUBMITTED", "REVIEWED", 5) | ("SUBMITTED", "REVIEWED", 1) => self
                .repo
                .update_inspection_status(inspection_id, "REVIEWED", user_id)
                .await
                .map_err(|e| e.to_string()),
            // Kasubsie / Admin menyetujui
            ("REVIEWED", "APPROVED", 6)
            | ("REVIEWED", "APPROVED", 1)
            | ("SUBMITTED", "APPROVED", 1) => self
                .repo
                .update_inspection_status(inspection_id, "APPROVED", user_id)
                .await
                .map_err(|e| e.to_string()),
            // Penolakan
            (_, "REJECTED", 1) | (_, "REJECTED", 6) | (_, "REJECTED", 5) => self
                .repo
                .update_inspection_status(inspection_id, "REJECTED", user_id)
                .await
                .map_err(|e| e.to_string()),
            _ => {
                Err("Transisi status tidak diizinkan untuk Role Anda atau urutan salah".to_string())
            }
        }
    }
}
