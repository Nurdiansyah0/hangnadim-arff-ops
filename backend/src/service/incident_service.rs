...
impl IncidentService {
    ...
    pub async fn create_incident(
        ...
        self.repo
            .create_incident(None, description, location, severity, photo_url)
            .await
            .map_err(|e| e.to_string())
    }
...
mod tests {
    ...
    async fn test_create_incident_success() {
        ...
        let res = service
            .create_incident("Fire on RWY 04", Some("RWY 04"), Some("HIGH"), None)
            .await;
        assert!(res.is_ok());
    }
    ...
    async fn test_create_incident_empty_desc() {
        ...
        let service = IncidentService::new(repo);
        let res = service
            .create_incident("   ", Some("RWY 04"), Some("HIGH"), None)
            .await;
        assert!(res.is_err());
    }
}
