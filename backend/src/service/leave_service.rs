...
impl LeaveService {
    ...
    pub async fn submit_request(
        ...
        let personnel = personnel_repo.get_by_id(personnel_id).await.map_err(|e| e.to_string())?;
        let team = personnel.shift.ok_or("Personel tidak memiliki penugasan tim/shift")?;

        // 2. Fetch holidays for the year(s)
        let holidays = crate::service::holiday_service::HolidayService::get_holidays(start_date.year()).await.unwrap_or_default();
        ...
        while current <= end_date {
            ...
            if is_working {
                working_days += 1;
                let count = self.repo.count_overlapping_requests_by_team(&team, current).await?;
                if count >= 2 {
                    return Err(format!(
        ...
        if remaining < working_days as i32 {
            return Err(format!(
    ...
    pub async fn process_request(
        ...
        if status == "APPROVED" {
            ...
            let personnel_id = Uuid::parse_str(req["personnel_id"].as_str().unwrap_or_default()).map_err(|e| e.to_string())?;
            let start_date = chrono::NaiveDate::parse_from_str(req["start_date"].as_str().unwrap_or_default(), "%Y-%m-%d").map_err(|e| e.to_string())?;
            let end_date = chrono::NaiveDate::parse_from_str(req["end_date"].as_str().unwrap_or_default(), "%Y-%m-%d").map_err(|e| e.to_string())?;
            ...
            while current <= end_date {
                let is_working = if team == "Normal" {
                    current.weekday() != Weekday::Sat && current.weekday() != Weekday::Sun && !holidays.contains(&current)
                } else {
                    let delta = (current - baseline_date).num_days();
                    let cycle_idx = if delta >= 0 { delta % 6 } else { (6 + (delta % 6)) % 6 };
                    let (m_team, n_team) = match cycle_idx {
                        0 => ("Alpha", "Charlie"), 1 => ("Bravo", "Alpha"), 2 => ("Bravo", "Alpha"),
                        3 => ("Charlie", "Bravo"), 4 => ("Charlie", "Bravo"), 5 => ("Alpha", "Charlie"),
                        _ => unreachable!(),
                    };
                    team == m_team || team == n_team
                };
            ...
            personnel_repo.deduct_leave(personnel_id, working_days).await.map_err(|e| e.to_string())?;
        }
