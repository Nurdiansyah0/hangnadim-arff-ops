...
async fn update_assignment(
    ...
    if role_id != 1 && role_id != 3 && role_id != 5 {
        return Err((StatusCode::FORBIDDEN, "Unauthorized role for assignment update".to_string()));
    }
...
async fn generate_monthly(
    ...
    let role_id = claims.role_id.unwrap_or(0);
    if role_id != 1 && role_id != 3 && role_id != 5 {
        return Err((StatusCode::FORBIDDEN, "Unauthorized role for roster generation".to_string()));
    }
