...
async fn upload_chunk(
    ...
    while let Some(field) = multipart.next_field().await.map_err(|e| (StatusCode::BAD_REQUEST, e.to_string()))? {
        ...
        if name == "upload_id" {
            ...
            chunk_index = field.text().await.unwrap_or_else(|_| "0".to_string()).parse().unwrap_or(0);
        } else if name == "total_chunks" {
            total_chunks = field.text().await.unwrap_or_else(|_| "0".to_string()).parse().unwrap_or(0);
        } else if name == "file_name" {
            file_name = field.text().await.unwrap_or_default();
        } else if name == "chunk" {
            chunk_data = field.bytes().await.map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?.to_vec();
        }
    }

    if upload_id.is_empty() || chunk_data.is_empty() {
        return Err((StatusCode::BAD_REQUEST, "Missing upload metadata or data".to_string()));
    }
    ...
    fs::write(&chunk_path, chunk_data).await.map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;

    // Check if all chunks are uploaded
    let mut uploaded_count = 0;
    let mut dir = fs::read_dir(&temp_dir).await.map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    while let Some(_) = dir.next_entry().await.map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))? {
        uploaded_count += 1;
    }
    ...
    if uploaded_count == total_chunks {
        ...
        fs::create_dir_all(final_dir).await.map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
        ...
        for i in 0..total_chunks {
            ...
            let part_data = fs::read(&part_path).await.map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
            use tokio::io::AsyncWriteExt;
            final_file.write_all(&part_data).await.map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
        }
