...
impl AuditRepoTrait for AuditRepository {
    async fn get_logs(&self, offset: i64, limit: i64) -> Result<(Vec<AuditLog>, i64), Error> {
        let logs: Vec<AuditLog> = sqlx::query_as(r#"
            SELECT 
                a.*,
            ...
            "UPDATE" => {
                if let Some(data) = original {
                    ...
                    if let Some(record_id) = data.get("id").and_then(|v| v.as_str()) {
                        let _query = format!("UPDATE {} SET (id, updated_at) = (id, NOW()) WHERE id = '{}'", table, record_id);
                        ...
                        sqlx::query(&format!("INSERT INTO {} SELECT * FROM jsonb_populate_record(NULL::{}, $1)", table, table))
                            .bind(data)
                            .execute(&mut *tx).await?;
                    }
                }
